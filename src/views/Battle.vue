  ...
  <div class="flex flex-col justify-center items-center flex-wrap">
    <div class="flex gap-2 w-full p-2 max-w-7xl">
      <div class="w-1/3 p-2">
        ...
        <ul class="scrollable">
          <li v-for="turn in turns.sort((a, b) => b.initiative - a.initiative)" :key="turn.name" class="w-full p-2 mb-1 border-2 rounded-md flex gap-1 justify-between text-white select-none cursor-grab items-center" :class="{ 'border-darkgreen bg-lightgreen': turn.player, 'border-red-500 bg-red-400': !turn.player }">
            <div>
            ...
            <div class="flex gap-1 items-center">
              ...
              <button class="text-white rounded-md px-1 py-1 text-xs font-bold cursor-pointer" :class="{ 'bg-darkgreen': turn.player, 'bg-red-700': !turn.player }" @click="turns.splice(turns.indexOf(turn), 1)">
...
const turns = ref([
  { name: 'Mirko', initiative: 20, player: true },
  { name: 'Gianluca', initiative: 15, player: true },
  { name: 'Daniele', initiative: 10, player: true },
  { name: 'Alessandro', initiative: 5, player: true },
])

onMounted(() => {
  // generate random enemies from 5 to 10 with random initiative
  const enemies = Math.floor(Math.random() * 6) + 5
